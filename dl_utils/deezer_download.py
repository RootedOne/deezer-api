...
def deezer_search(search, search_type):
    ...
    # search_type: either one of the constants: TYPE_TRACK|TYPE_ALBUM|TYPE_ALBUM_TRACK|TYPE_ARTIST (TYPE_PLAYLIST is not supported)
    ...
    try:
        ...
        raise DeezerApiException(f"Could not search for track/album/artist '{search}': {e}") from e
    return_nice = []
    for item in data:
        i = {}
